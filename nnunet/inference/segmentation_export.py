...
def writeSlices(series_tag_values, save_path, new_img, i):
    ...
    writer.SetFileName(os.path.join(save_path,str(i)+'.dcm'))
    writer.Execute(image_slice)

def save_segmentation_dicom_from_softmax(segmentation_softmax: Union[str, np.ndarray], out_fname: str,
                                         properties_dict: dict, order: int = 1,
                                         region_class_order: Tuple[Tuple[int]] = None,
                                         seg_postprogess_fn: callable = None, seg_postprocess_args: tuple = None,
                                         resampled_npz_fname: str = None,
    ...
    direction = seg_resized_itk.GetDirection()
    ...
    series_tag_values = series_tag_values + properties_dict['tags']
    print(series_tag_values)

    if not os.path.isdir(out_fname):
        os.mkdir(out_fname)
    print(out_fname)
    list(map(lambda i: writeSlices(series_tag_values, out_fname, seg_resized_itk, i), range(seg_resized_itk.GetDepth())))
    ...
    if (non_postprocessed_fname is not None) and (seg_postprogess_fn is not None):
        ...
        seg_resized_itk.SetOrigin(properties_dict['itk_origin'])
        seg_resized_itk.SetDirection(properties_dict['itk_direction'])
        if not os.path.isdir(non_postprocessed_fname):
            os.mkdir(non_postprocessed_fname)
        list(map(lambda i: writeSlices(series_tag_values, non_postprocessed_fname, seg_resized_itk, i), range(seg_resized_itk.GetDepth())))
        #sitk.WriteImage(seg_resized_itk, non_postprocessed_fname)
